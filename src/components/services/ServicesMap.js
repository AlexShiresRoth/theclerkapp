...
const ServicesMap = ({ scrollServicesDown, scrollServicesUp, currentIndex, intersecting }) => {
	...
	const onTouchEnd = (e, i) => {
		const diff = Math.abs(startPos - e.changedTouches[0].clientY);
		if (diff > 100 && intersecting)
			return e.changedTouches[0].clientY < startPos
				? i < services.length - 1
