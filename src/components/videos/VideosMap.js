...
export const VideosMap = () => {
	...
	const handleScrollingRight = e => {
		sliderRefs.current[sliderNum].scrollLeft = sliderRefs.current[sliderNum].scrollLeft += 20;
	...
	const handleMouseLeave = e => cancelAnimationFrame(animationRef.current);
	...
	const videoMap = videos.map((video, i, arr) => {
		return (
			<div className={videoStyles.video__container} onMouseEnter={e => setSliderNum(i)}>
				...
				<div className={videoStyles.slider__controls}>
					<button onMouseDown={e => handleScrollingLeft()} onMouseLeave={() => handleMouseLeave()}>
						<FiChevronLeft />
					</button>
					<p>Video Count: {arr[i].videos.length}</p>
					<button onMouseDown={e => handleScrollingRight()} onMouseLeave={() => handleMouseLeave()}>
						<FiChevronRight />
					</button>
				</div>
			</div>
